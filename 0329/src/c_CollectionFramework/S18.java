//package c_CollectionFramework;
//
//
//import java.util.ArrayList;
//import java.util.Arrays;
//import java.util.List;
//
//public class S18 {
//	public static void main(String[] args) {
//		
//		List<Integer> aList1 = new ArrayList<Integer>();
//		
//		//#1. add(E element)
//		aList1.add(3);
//		aList1.add(4);
//		aList1.add(5);
//		System.out.println(aList1.toString()); //=~.println(aList1) [3, 4, 5]
//		
//		//#2. add(int index, E element)//기존 값이 변하는게 아닌, 위치한 인덱스 값에 추가됨. 뒤에 인덱스 값이 있을경우 한칸씩 밀림
//		aList1.add(1, 6);
//		System.out.println(aList1.toString()); //[3, 6, 4, 5]
//		
//		//#3. addAll(또 다른 리스트 객체)//같은 유형의 리스트 합치기
//		List<Integer> aList2 = new ArrayList<>();
//		aList2.add(1);
//		aList2.add(2);
//		aList2.addAll(aList1);
//		System.out.println(aList2); //[1, 2, 3, 6, 4, 5]//aList1 = [1,2] + aList2 = [3,6,4,5]  
//		
//		//#4. addAll(int index, 또 다른 리스트 객체)
//		List<Integer> aList3 = new ArrayList<>();
//		aList3.add(1);
//		aList3.add(2); //[1 2]
//		aList3.addAll(1, aList3);//인덱스 1위치부터 aList3을 추가함.
//		System.out.println(aList3); //[1, 1, 2, 2]
//		
//		//#5. set(int index, E element)//인덱스에 위치한 값을 추가하는것이 아닌, 기존 인덱스값을 변경하는 키워드
//		aList3.set(1,5);
//		aList3.set(3,6);
//		//aList3.set(4,7); //java.lang.IndexOutOfBoundsException:		
//		System.out.println(aList3); //[1, 5, 2, 6]
//		
//		//#6. remove(int index)
//		aList3.remove(1); //1번 인덱스를 삭제해라
//		System.out.println(aList3); //[1, 2, 6]
//		
//		//#7. remove(Object o)
//		aList3.remove(new Integer(2));//추후 사라질 방식, 권장하지 않음.
//		System.out.println(aList3); //[1, 6]
//		
//		//#8. clear()
//		aList3.clear();
//		System.out.println(aList3); //[]
//		
//		//#9. isEmpty()//리스트 내부가 Empty > 비어있냐? > boolean(true,false)출력
//		System.out.println(aList3.isEmpty()); //true
//		
//		//#10. size()//리스트의 길이, 배열의 길이는 .length
//		System.out.println(aList3.size());//0
//		aList3.add(1);
//		aList3.add(2);
//		aList3.add(3);
//		System.out.println(aList3); //[1, 2, 3]
//		System.out.println(aList3.size());//3
//		
//		//#11.get(int index)//해당 인덱스의 위치한 값 불러옴.
//		System.out.println("0번째 : " + aList3.get(0)); //1
//		System.out.println("1번째 : " + aList3.get(1)); //2
//		System.out.println("2번째 : " + aList3.get(2)); //3
//		for(int i=0; i<aList3.size(); i++)
//			System.out.println(i+"번째 : "+aList3.get(i));
//		
//		//#12. toArray() List-->Array
//		Object[] object = aList3.toArray();
//		System.out.println(Arrays.toString(object)); //[1, 2, 3]
//		
//		//#13-1. toArray(T[] t) -->  T[]
//		Integer[] integer1 = aList3.toArray(new Integer[3]);
//		System.out.println(Arrays.toString(integer1));
//		
//		//#13-2. toArray(T[] t) -->  T[]
//		Integer[] integer2 = aList3.toArray(new Integer[5]);
//		System.out.println(Arrays.toString(integer2));				
//	}
//}